...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ApiErrorResponse> handleResponseStatus(ResponseStatusException ex,
        ...
        String message =
                ex.getReason() != null ? ex.getReason() : messages.get("error.request.failed");

        return ResponseEntity.status(statusCode)
                .body(new ApiErrorResponse(statusCode.value(),
                        status != null ? status.getReasonPhrase() : "Error", message,
    ...
    public ResponseEntity<ApiErrorResponse> handleGeneric(Exception ex,
        ...
        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                .body(new ApiErrorResponse(HttpStatus.INTERNAL_SERVER_ERROR.value(),
                        ...
                        messages.get("error.unexpected"), request.getRequestURI(), Instant.now()));
    }

    private String formatFieldError(FieldError fieldError) {
        String field = fieldError.getField();
        String message = fieldError.getDefaultMessage() != null ? fieldError.getDefaultMessage()
                : messages.get("error.invalid");
