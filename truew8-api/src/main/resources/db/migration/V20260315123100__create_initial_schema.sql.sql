-- Tabela de Usuários (Core do TrueW8 e controle do PLG)
CREATE TABLE users (
    ...
    provider_id VARCHAR(255), -- ID único do Google/Apple
    plan_type VARCHAR(50) DEFAULT 'FREE', -- 'FREE', 'PRO'
    ocr_operations_count INT DEFAULT 0, -- Trava para as 2 operações gratuitas
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP
);

-- Catálogo Global de Ativos (Tickers únicos)
CREATE TABLE assets (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
-- Composição das Carteiras Recomendadas (Cruzamento Ativo x Peso Alvo)
...
-- Assinaturas do Usuário (O peso que o usuário dá para cada carteira no patrimônio global)
...
-- Posição Atual do Usuário (O que ele tem custodiado na corretora para calcular a diferença)
CREATE TABLE user_holdings (
    user_id UUID REFERENCES users(id) ON DELETE CASCADE,
    asset_id UUID REFERENCES assets(id) ON DELETE CASCADE,
    current_quantity DECIMAL(15,4) DEFAULT 0,
    average_price DECIMAL(15,4) DEFAULT 0, -- Opcional para a V1, útil no futuro
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
